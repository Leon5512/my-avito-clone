...
- Создайте проект React с помощью create-vite: `npm init --yes vite@latest my-avito-clone -- --template=react  --no-rolldown --no-immediate`.
- Обновление `vite.config.js` с портом.
- Создание файла проекта (`my-avito-clone.esproj`).
